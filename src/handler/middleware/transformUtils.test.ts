...
import {
  ...
  replaceStep,
  tagStep,
} from '@/src/handler/middleware/transformUtils';
...
describe('transformUtils helpers', () => {
  ...
  it('insertBefore/After produce correct order', () => {    const list = [mk('head'), mk('shape'), mk('serializer')];
    const x = mk('zod-after');
    const before = insertBefore(list, 'serializer', x);
    expect(before.map(getId)).toEqual(['head', 'shape', 'zod-after', 'serializer']);
    const after = insertAfter(list, 'shape', x);
    expect(after.map(getId)).toEqual(['head', 'shape', 'zod-after', 'serializer']);
  });
