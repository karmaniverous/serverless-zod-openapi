...
// keep .ts extension for robust ESM resolution under tsx
...
const makeImports = (root: string, outDir: string, files: string[]): string[] =>
  files.map((abs) => {
    ...
    // Ensure explicit .ts extension for ESM resolution
    const withExt = relFromOut.endsWith('.ts')
      ? relFromOut
      : `${relFromOut}.ts`;
    const spec = withExt.startsWith('.') ? withExt : `./${withExt}`;
    return `import '${spec}';`;
  });
