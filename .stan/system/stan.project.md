...
- Published outputs (ESM/CJS/DTS) MUST NOT contain project-local TS path
  aliases (e.g., `@/src/...`). The Rollup build resolves `@/` to `src/`
  via @rollup/plugin-alias so downstream runtimes never see alias specifiers.

## 1) Repository scope and publishing

- Publish only the toolkit and templates to npm:  - files: ["dist", "templates"]
...
- App.create(config) captures:
  ...
  - Build policy (alias resolution for `@/` during publish).
  - Implementation‑wide Serverless defaults.
  - Stage artifacts: provider environment, params, and a per‑function env builder.
- Define functions in small modules (lambda.ts), register OpenAPI (openapi.ts),  and add non‑HTTP extras (serverless.ts) when applicable.
